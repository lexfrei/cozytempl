...
func registerPageRoutes(pgh *handler.PageHandler) *http.ServeMux {
	...
	pageMux.HandleFunc("POST /tenants/{tenant}/apps/{name}/actions/{action}", pgh.InvokeAction)
	...
	pageMux.HandleFunc("POST /fragments/app-yaml", pgh.AppFormYAMLFragment)
	pageMux.HandleFunc("POST /fragments/app-yaml-to-form", pgh.AppFormYAMLToFormFragment)
